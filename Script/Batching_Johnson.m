 ...
 % @LastEditTime: 2022-03-02 22:52:12
 % @FilePath: \Code\Script\Batching_Johnson.m
 % @Description: 
 % @
 % @Copyright (c) 2022 by Scott Zheng, All Rights Reserved. 
 %% 
function [Batch, Cmax, TEC] = Batching_Johnson(Instance, B, PlotSwitch)
    ...
    L = [];
    while ~isempty(Jobs)
        [row, col] = find(Instance(3:4, Jobs) == min(min(Instance(3:4,Jobs)))); % col可能重复小心
        if row(1) == 1
            L =[L, Jobs(col(1))];
        else
            R =[R, Jobs(col(1))];    
